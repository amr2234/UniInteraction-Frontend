...
export const useToggleDepartmentStatus = () => {
  ...
  return useMutation<DepartmentDto, ApiError, number>({
    mutationFn: departmentsApi.toggleDepartmentStatus,
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.departments.all });
      toast.success(data.isActive ? 'تم تفعيل القسم بنجاح' : 'تم إلغاء تفعيل القسم بنجاح');
    },
    onError: (error) => {
      toast.error('فشل تغيير حالة القسم', {
        description: error.message,
      });
    },
  });
